...
problem = createOptimProblem('fmincon', 'objective', @(x) OCV_stoichiometry_model_06(x,OCP_n,OCP_p,OCV,w), ...
...
% plot(OCV2(:,1),OCV_hat3,'m-','LineWidth',lw,'MarkerSize',msz); hold on
% 
% % 
% % pos = get(gcf, 'Position');
% % set(gcf, 'Position', [pos(1) pos(2) width*100, height*100]); %<- Set size
% % set(gca, 'FontSize', fsz, 'LineWidth', alw); %<- Set properties
% % 
% legend('FCC data','FCC fit','FCC(2) data','FCC(2) fit','Location', 'none', 'Position', [0.7 0.85 0.1 0.05])
% xlabel('SOC');
% ylabel('OCV (V)');
% title('OCV1 vs. OCV2 (0.01C vs. 0.05C)');
